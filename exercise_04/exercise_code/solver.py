...
class Solver(object):
    ...
    def _step(self):
        ...
        dw = self.model.backward(y_out) #(dhat{y} / dW)
        _, dy = self.loss_func.__call__(y_out, y_train) #(dLoss / dhat{y})
        
        #loss_gradient = np.zeros(X_train.shape)
        #loss_gradient = dLoss / dW = (dLoss / dhat{y}) * (dhat{y} / dW)
        #loss_gradient += dy * dw
        #loss_gradient /= X_train.shape[0]
        
        loss_gradient = dy * dw
        
        self.opt.step(loss_gradient)
        """
